...
# ── FII/DII shortcut ─────────────────────────────────────────
def run_fii_dashboard(tickers=None):
    """Shortcut to run FII/DII dashboard from main.py"""
    from fii_dii_dashboard import run_fii_dii_dashboard
    run_fii_dii_dashboard(stock_tickers=tickers)


if __name__ == "__main__":
    import sys
    if len(sys.argv) > 1 and sys.argv[1] == "fii":
        tickers = sys.argv[2].split(",") if len(sys.argv) > 2 else None
        run_fii_dashboard(tickers)
    else:
        main()
