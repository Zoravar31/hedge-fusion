...
def parse_json_response(raw: str) -> dict:
    ...
    Handles markdown code fences, nested JSON, and partial responses.
    """
    import re
    if not raw:
        return {}
    ...
    # Last resort: find ANY {...} block from largest to smallest
    try:
        for match in sorted(re.finditer(r"\{[^{}]*(?:\{[^{}]*\}[^{}]*)*\}", cleaned, re.DOTALL),
